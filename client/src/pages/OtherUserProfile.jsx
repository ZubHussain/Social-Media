...
const OtherUserProfile = () => {
    ...
    const fetchUser = async () => {
      try {
        ...
        if (response.ok) {
          ...
          const loggedInUser = await fetch("http://localhost:3000/user/profile", {
            headers: {
              Authorization: `Bearer ${localStorage.getItem("token")}`,
            },
          });
  ...
  const followUser = async () => {
    try {
      const response = await fetch(`http://localhost:3000/user/follow/${userId}`, {
        method: "POST",
        headers: {
          Authorization: `Bearer ${localStorage.getItem("token")}`,
        },
  ...
  const unfollowUser = async () => {
    try {
      const response = await fetch(`http://localhost:3000/user/unFollow/${userId}`, {
        method: "POST",
        headers: {
          Authorization: `Bearer ${localStorage.getItem("token")}`,
        },
